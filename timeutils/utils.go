...
// NewNow initialize Now with time
...
// EndOfNextMonth end of month
func EndOfNextMonth() time.Time {
	return With(time.Now()).EndOfNextMonth()
}

// EndOfNextMonth end of month
func NumDaysOfMonth() int {
...
func (now *Now) EndOfNextMonth() time.Time {
	return now.BeginningOfMonth().AddDate(0, 2, 0).Add(-time.Nanosecond)
}
...
func (now *Now) EndOfSunday() time.Time {
	return NewNow(now.Sunday()).EndOfDay()
}
